...
package com.ramotion.cardslider.examples.simple.detection.tracking;
...
import com.ramotion.cardslider.examples.simple.detection.env.BorderedText;
import com.ramotion.cardslider.examples.simple.detection.env.ImageUtils;
import com.ramotion.cardslider.examples.simple.detection.env.Logger;
import com.ramotion.cardslider.examples.simple.detection.tflite.Classifier.Recognition;
...
public class MultiBoxTracker {
  ...
  public synchronized void draw(final Canvas canvas) {
    ...
    final Paint boxPaintLab = new Paint();
    boxPaintLab.setColor(Color.BLUE);
    boxPaintLab.setStyle(Style.FILL);
    ...
    for (final TrackedRecognition recognition : trackedObjects) {
      ...
      if(!TextUtils.isEmpty(recognition.title)){
        switch (recognition.title){
          case "avenue" : DetectString="L'horloge de l'avenue"; break;
          case "diwan" : DetectString="porte diwan Sfax"; break;
          case "bourgiba" :DetectString="H.Bourguiba"; break;
        }
      }
      final String labelString =
          !TextUtils.isEmpty(recognition.title)
              ? String.format("%s %.2f", DetectString, (100 * recognition.detectionConfidence))
              : String.format("%.2f", (100 * recognition.detectionConfidence));
      //            borderedText.drawText(canvas, trackedPos.left + cornerSize, trackedPos.top,
      // labelString);
//      borderedText.drawText(
//          canvas, trackedPos.left + cornerSize, trackedPos.top, labelString + "%", boxPaint);
      DetectLabel =labelString;
    }

    for (final Pair<Float, RectF> detection : screenRects) {
      final RectF rect = detection.second;
      canvas.drawRect(rect, boxPaint);
      canvas.drawRect((new RectF(rect.left,(rect.top),rect.right,(rect.top+64f))), boxPaintLab);
      canvas.drawText(DetectLabel, rect.left, (rect.top+60f), textPaint);
    }
